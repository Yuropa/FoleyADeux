...
def compute_RAFT_optical_flow(prev_batch, curr_batch, model, device, bound=20):
...
def cal_for_frames(video_path, output_dir, n_frames, width, height, batch_size, device):
    transforms, model = load_model_Raft(device)
    ...
    for idx in range(0, len(frames)-1, batch_size):
        ...
        flows, rescaled_flows = compute_RAFT_optical_flow(prev_frames, curr_frames, model, device) # bound='max'
