...
def pipline_align(video_path, output_dir):
    ...
    audio_path = P.join(ori_audio_dir, audio_name)

    os.system(f'ffmpeg -i "{video_path}" -loglevel error -f wav -vn -y "{audio_path}"')

    align_video_dir = P.join(output_dir, "videos_align")
    os.makedirs(align_video_dir, exist_ok=True)

    duration = execCmd(f'ffmpeg -i "{audio_path}" 2>&1 | grep \'Duration\' | cut -d \' \' -f 4 | sed s/,//')
...
def pipline_cut(video_id, metadata_dir, preproc_dir, output_dir, fps, sr, duration_target):
    video_name, onset_id, material = video_id.rsplit("_", 2)
    ...
    os.system('ffmpeg -ss {} -to {} -i "{}" -loglevel error -c:v libx264 -c:a aac -strict experimental -b:a 98k -y "{}"'.format(
        ...
        P.join(preproc_dir, "videos_align", video_name+".mp4"),
        P.join(cut_video_dir, video_name+'_'+onset_id+".mp4")))
